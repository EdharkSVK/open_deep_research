...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run tests for Open Deep Research with rich console output"
    )
    parser.add_argument(
        "--rich-output",
        action="store_true",
        default=True,
        help="Show rich output in terminal (default: True)",
    )
    ...
    parser.add_argument(
        "--supervisor-model",
        help="Model for supervisor agent (e.g., 'groq:llama-3.3-70b-versatile')",
    )
    parser.add_argument(
        "--researcher-model",
        help="Model for researcher agent (e.g., 'groq:deepseek-r1-distill-llama-70b')",
    )
    parser.add_argument(
        "--planner-provider", help="Provider for planner model (e.g., 'groq')"
    )
    parser.add_argument(
        "--planner-model",
        help="Model for planner in graph-based agent (e.g., 'claude-3-7-sonnet-latest')",
    )
    parser.add_argument(
        "--writer-provider", help="Provider for writer model (e.g., 'groq')"
    )
    parser.add_argument(
        "--writer-model",
        help="Model for writer in graph-based agent (e.g., 'llama-3.3-70b-versatile')",
    )
    parser.add_argument(
        "--eval-model",
        help="Model for evaluating report quality (default: openai:claude-3-7-sonnet-latest)",
    )
    parser.add_argument(
        "--max-search-depth", help="Maximum search depth for graph agent"
    )
...
def test_graph_basic_execution():
    ...
    def compile_report(state, config):
    ...
    sg.add_node("compile_final_report", compile_report)
    sg.add_edge(START, "compile_final_report")
    sg.add_edge("compile_final_report", END)
    g = sg.compile()

    import asyncio

    result = asyncio.run(g.ainvoke({"topic": "x"}))
