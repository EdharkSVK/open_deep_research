    ...
    if research_agent == "multi_agent":
        return {
            "supervisor_model": (
                ...
                or os.environ.get("SUPERVISOR_MODEL", "qwen/qwen3-32b")
            ),
            "researcher_model": (
                request.config.getoption("--researcher-model")
                or os.environ.get("RESEARCHER_MODEL", "qwen/qwen3-32b")
            ),
        }
    else:  # graph agent
        return {
            "planner_model": (
                ...
                or os.environ.get("PLANNER_MODEL", "qwen/qwen3-32b")
            ),
            "writer_model": (
                request.config.getoption("--writer-model")
                or os.environ.get("WRITER_MODEL", "qwen/qwen3-32b")
            ),
    ...
    if research_agent == "multi_agent":
        ...
        thread = {
            "configurable": {
                "thread_id": str(uuid.uuid4()),
                "search_api": search_api,
                "planner_model": models.get("planner_model", "qwen/qwen3-32b"),
                "writer_model": models.get("writer_model", "qwen/qwen3-32b"),
