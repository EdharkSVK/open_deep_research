...
class ReportStateInput(MessagesState):
    ...
    already_clarified_topic: Optional[bool] = (
        None  # If the user has clarified the topic with the agent
    )
...
class ReportState(MessagesState):
    ...
    feedback_on_report_plan: Annotated[
        tuple[str, ...],
        operator.add,
    ]  # Tuple of feedback on the report plan
    sections: list[Section]  # List of report sections
    completed_sections: Annotated[tuple[Section, ...], operator.add]  # Send() API key
...
class SectionOutputState(MessagesState):
    ...
    # for evaluation purposes only
    # this is included only if configurable.include_source_str is True    source_str: str  # String of formatted source content from web search
