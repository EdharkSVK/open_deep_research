...
from open_deep_research.tools import crawl_url, tavily_search
...
async def _load_mcp_tools(
    ...
    for mcp_tool in mcp_tools:
        ...
        if mcp_tool.name in existing_tool_names:
            warnings.warn(
                f"Trying to add MCP tool with a name {mcp_tool.name} that is already in use - this tool will be ignored."
            )
            continue

        if (
            configurable.mcp_tools_to_include
            and mcp_tool.name not in configurable.mcp_tools_to_include
        ):
            continue

        filtered_mcp_tools.append(mcp_tool)
...
async def supervisor_tools(
    ...
    if sections_list:
        ...
        result.append(
            {
                "role": "user",
                "content": "Introduction written. Now write a conclusion section.",
            }
        )
        state_update = {
