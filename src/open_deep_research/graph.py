...
async def generate_report_plan(state: ReportState, config: RunnableConfig):
    ...
    plan = (
        await planner_llm.ainvoke([SystemMessage(content=system_instructions_sections)])
    ).sections
...
def human_feedback(
    ...
    if not any(sec.research for sec in sections):
        return Command(goto="write_section", update={"plan": sections})
    sections_str = "\n\n".join(
